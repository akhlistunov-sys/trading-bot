...
КОНТЕКСТ:
- Время: {time_context}
- Ключевые тикеры MOEX: {liquid_tickers} (для ориентира)
...
2. Если это сигнал — найди ВСЕ российские акции (тикеры MOEX), упомянутые в новости.
3. Если новость общая (о рынке, экономике, секторе) — выбери ключевые тикеры из списка выше, на которые она влияет.
4. Оцени влияние на цену: 'positive' (рост), 'negative' (падение), 'neutral' (неопределенно).
5. Оцени силу влияния от 1 (минимально) до 10 (максимально).
6. Если сигнал слабый или не торгуемый — верни is_tradable: false.
